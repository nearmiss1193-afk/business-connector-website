 ...
 * Run with: node sync-properties.mjs
 */

import { drizzle } from 'drizzle-orm/mysql2';
import { eq } from 'drizzle-orm';
import * as schema from './drizzle/schema-properties.ts';
    ...
    for (let i = 0; i < realtorProperties.length; i++) {
      ...
      try {
        ...
        const existing = await db
          ...
          .where(eq(schema.properties.mlsId, mappedProperty.mlsId))
          .limit(1);
        ...
        if (existing.length > 0) {
          ...
          const result = await db
            ...
            .values(mappedProperty);

          const propertyId = result.insertId;

          // Insert property images
          if (images && images.length > 0) {
            const imageRecords = images.slice(0, 20).map((url, index) => ({
              propertyId,
              imageUrl: url,
              order: index + 1,
              createdAt: new Date(),
            ...
            await db.insert(schema.propertyImages).values(imageRecords);
