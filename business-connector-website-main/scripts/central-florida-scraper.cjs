...
const CONFIG = {
  ...
  // Base44 configuration for uploads
  base44: {
...
function formatRealtyInUSProperty(prop, location) {
  ...
  return {
    // Basic Info
    property_id: prop.property_id || `realty_${Date.now()}_${Math.random()}`,
    source: 'realty_in_us',
    ...
    zip_code: address_data.postal_code || location.zip,
    ...
    square_feet: description.sqft || 0,
    lotSize: description.lot_sqft || 0,
    yearBuilt: description.year_built || null,
    property_type: description.type || 'Single Family',
    ...
    // Images & URLs
    image_url: prop.primary_photo?.href || '',
    images: (prop.photos || []).map(img => img.href).filter(Boolean),
    listing_url: prop.href || '',
    virtual_tour_url: prop.virtual_tours?.[0]?.href || null,
...
async function searchZillow(location) {
  try {
    ...
    console.error(`  ❌ Zillow error:`, error.response?.status, error.message);
    return [];
  }
}
...
function formatZillowProperty(prop, location) {
  return {
    property_id: prop.zpid || `zillow_${Date.now()}_${Math.random()}`,
    source: 'zillow',
    address: prop.address || 'Address not available',
    city: location.city,
    state: location.state,
    zip_code: prop.zipcode || location.zip,
    region: location.region,
    ...
    property_type: prop.homeType || 'Single Family',
...
async function uploadToBase44(property) {
  try {
    await axios.post(
      `https://app.base44.com/api/apps/${CONFIG.base44.appId}/entities/${CONFIG.base44.entity}`,
      property,
      {
        headers: {
...
function exportToCSV(properties, filename) {
  ...
  const headers = [
    'property_id', 'source', 'address', 'city', 'state', 'zip_code', 'region',
    'price', 'bedrooms', 'bathrooms', 'square_feet', 'lotSize', 'yearBuilt', 'property_type',
    'listingStatus', 'daysOnMarket', 'mlsNumber', 'listingId',
    'image_url', 'listing_url', 'photoCount',
...
async function main() {
  ...
  for (const market of CONFIG.markets) {
    ...
    try {
      ...
      // Upload to Base44 in real-time
      for (const prop of realtyProps) {
        const uploaded = await uploadToBase44(prop);
  ...
  console.log(`  ☁️  Uploaded to Base44: ${totalUploaded}`);
  ...
  const summary = {
    ...
    uploadedToBase44: totalUploaded,
    scrapedAt: new Date().toISOString(),
  ...
  console.log('\nNext: Check centralfloridahomes.com! 🚀');
}
